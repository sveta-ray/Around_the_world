...
public class Quiz8 {
    ...
    public Quiz8(MyFrame frame) throws IOException {
        this.frame = frame;
        image = ImageIO.read(new File("data\\Quiz8.png"));
        code.add(49);
        code.add(51);
        code.add(48);
        code.add(10);
...
//
        if(codec.size() == 1){
            ...
            g.drawString(Integer.toString (codec.get(0)-48), 580, 780);
        }
        if(codec.size() == 2){
            g.setFont(new Font("TimesRoman", Font.PLAIN, 100));
            g.drawString(Integer.toString (codec.get(0)-48), 580, 780);
            g.drawString(Integer.toString (codec.get(1)-48), 760, 780);
        }
        if(codec.size() == 3){
            g.setFont(new Font("TimesRoman", Font.PLAIN, 100));
            g.drawString(Integer.toString (codec.get(0)-48), 580, 780);
            g.drawString(Integer.toString (codec.get(1)-48), 760, 780);
            g.drawString(Integer.toString (codec.get(2)-48), 950, 780);
        }
